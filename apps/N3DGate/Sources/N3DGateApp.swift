// N3DGate — the iPhone gate of the Nemotron-3-Diarization Core AI port. Starts the gate on launch (no UI
// input), keeps the screen awake while it runs, shows progress and the result lines, and leaves
// Documents/n3d_gate/result.json + result.log for `devicectl device copy from` (../_run.sh).

import SwiftUI
import UIKit

@main
struct N3DGateApp: App {
...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Nemotron-3-Diarization gate").font(.headline)
            HStack {
