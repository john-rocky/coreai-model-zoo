...
- **CC BY-NC 4.0 — OpenJev (openjev/openjev)**: [`export_qwen3_5_decode_pipelined.py`](export_qwen3_5_decode_pipelined.py) adds optional `--name`, `--revision` and `--extra-metadata` with nil defaults, retaining the graph and quantization defaults; `_bundle.py` merges extra root metadata. `int8hu --head-sym --hf-id openjev/openjev --name openjev_27b_decode_int8hu_block32_sym --revision 5ec9e5fd2f80a6fff386779b1e5ac7e389971889 --extra-metadata models/openjev-27b/metadata-extra.json` exports the 64-layer text tower. [`letter/oracle_openjev.py`](letter/oracle_openjev.py) embeds 61 requests and uses the SHA-checked unchanged author helper over MLX BF16; bare-letter/chat/T=0.85 gates compare calibrated Noul at slope 1.829074, bias 0, preserving APUS/2B defaults. Mac GPU argmax 61/61, engines 122/122; measured through coreai-kit: parity 61/61, SemIf authored144 mean family balanced accuracy 0.907, contended times. [Card](../models/openjev-27b/README.md).
