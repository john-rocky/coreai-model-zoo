...
swift run -c release chat-cli --model lfm2.5-8b-a1b --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "lfm2.5-8b-a1b")
}
...
- First run downloads the model — ~9,000 MB (Mac) — then it loads from the
