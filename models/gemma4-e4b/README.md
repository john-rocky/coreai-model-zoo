...
swift run -c release chat-cli --model gemma-4-e4b --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "gemma-4-e4b")
}
...
- First run downloads the model — ~7,589 MB (Mac) — then it loads from the
