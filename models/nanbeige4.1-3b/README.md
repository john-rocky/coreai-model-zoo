...
swift run -c release chat-cli --model nanbeige4.1-3b --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "nanbeige4.1-3b")
}
...
- First run downloads the model — ~4,600 MB (Mac) / ~4,600 MB (iPhone) — then it loads from the
