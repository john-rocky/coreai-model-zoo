...
swift run -c release chat-cli --model gemma-4-31b --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "gemma-4-31b")
}
...
- First run downloads the model — ~20,121 MB (Mac) — then it loads from the
