...
swift run -c release chat-cli --model qwen3.6-35b-a3b --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "qwen3.6-35b-a3b")
}
...
- First run downloads the model — ~37,349 MB (Mac) — then it loads from the
