...
swift run -c release chat-cli --model lfm2.5-1.2b --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "lfm2.5-1.2b")
}
...
- First run downloads the model — ~1,702 MB (Mac) / ~1,702 MB (iPhone) — then it loads from the
