...
▶️ **Run it (source)** — the [Speak runner](https://github.com/john-rocky/coreai-kit/tree/0.7.3/Examples/Speak)
...
- First run downloads the model — ~1,374 MB (Mac) / ~1,378 MB (iPhone) — then it loads from the
...
| `ios/<name>/<name>.aimodel/` | The same 7 JIT bundles, laid out as `macos/` (every iPhone generation specializes them on its first load; Hub revision `38fc0aea`, 2026-09-26). iPhone 18 Pro, fresh install: base decode / prefill 1.13 / 1.19 s, feat decoder 1.06 s, the rest 0.09–0.19 s; 0.04–0.27 s on relaunch (load-only check) |
