...
swift run -c release chat-cli --model glm-4.7-flash --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "glm-4.7-flash")
}
...
- First run downloads the model — ~30,000 MB (Mac) — then it loads from the
