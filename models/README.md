...
| [`decider-0.8b`](decider-0.8b/README.md) | decider-0.8b (Mapika; System One decision model on Qwen3.5-0.8B-Base) — the zoo's **first decision model**: typed Choice / Score / Noul questions answered by calibrated option-letter probabilities in one pass, no generation | [🤗 decider-0.8b-CoreAI](https://huggingface.co/mlboydaisuke/decider-0.8b-CoreAI) | int8hu 1.34 GB (the Qwen3.5-0.8B ship recipe, HF id swapped); **probability gate vs the author's fp32 readout: argmax 44/44, max \|Δp\| 0.0084, mean of row means 0.00067**; engine argmax 44/44; **M4 Max decode 193.6 tok/s**; Swift reads the argmax only until the read-last-logits primitive exists (design in knowledge/) |
| [`openthai-systemone`](openthai-systemone/README.md) | OpenThai-SystemOne (iApp; Thai + English System One decision model on Qwen3.5-0.8B-Base with a 256-way slot head) — the zoo's **second decision model**: Choice (up to 255 options) / Score / Noul with an abstain probability, no generation | [🤗 OpenThai-SystemOne-CoreAI](https://huggingface.co/mlboydaisuke/OpenThai-SystemOne-CoreAI) | int8lin 1.07 GB (fp16 1.53 GB reference beside it); **probability gate vs the author's fp32 readout: argmax 50/50, max \|Δp\| 0.021, mean of row means 0.00066**; both Swift engines 200/200; M4 Max S=1 prefill proxy 252.7 tok/s; through coreai-kit tokens / argmax 50/50 and SemIf authored144 balanced 0.725 |
| [`laya-multilingual`](laya-multilingual/README.md) | laya multilingual (Convai Innovations; mmBERT-base 322M encoder + typed decision head) — the zoo's **first encoder-type decision model**: Choice / Score / Noul read at option markers in one forward pass, no prefill, no generation | [🤗 Laya-Multilingual-CoreAI](https://huggingface.co/mlboydaisuke/Laya-Multilingual-CoreAI) | wfp16 645 MB (fp16 weights, fp32 compute; fp32 1.29 GB reference beside it), S=256 / 512, one bundle with `main` + `act`; **oracle = the publisher's package, frozen fixture reproduced bit for bit; argmax 81/81, max \|Δp\| 4.5e-6 on 201 rows × 2 windows (Mac GPU)**; two-tier layer gate (1e-4 absolute / 2e-4 relative) with five negative controls; **M4 Max GPU 12.1 / 19.5 ms per question**, iPhone h18p GPU AOT compiled but not measured; through coreai-kit tokens / markers 201/201, 11.5 ms per decision, SemIf authored144 balanced 0.6114 (the model's own) |
| [`apus-openjev-v1-4b`](apus-openjev-v1-4b/README.md) | APUS-OpenJev-v1-4B (Qwen3.5-4B text tower; 32-layer decision readout from A–P logits under the chat template; LM head retained) | [🤗 APUS-OpenJev-v1-4B-CoreAI](https://huggingface.co/mlboydaisuke/APUS-OpenJev-v1-4B-CoreAI) | int8hu 5.77 GB, Mac GPU option argmax 48/48, max \|Δp\| 0.005302, both engines 96/96; measured through coreai-kit: 40/40 choice+noul parity, SemIf authored144 mean family balanced accuracy 0.906 |
| [`qwen3.5-2b-decision`](qwen3.5-2b-decision/README.md) | Jev-Style-Qwen3.5-2B-Decision (chaoliangUNSW/Jev-Style-Qwen3.5-2B-Decision-MLX-bf16; calibrated English typed decisions, LM head retained) | [🤗 Qwen3.5-2B-Decision-CoreAI](https://huggingface.co/mlboydaisuke/Qwen3.5-2B-Decision-CoreAI) | int8hu 3.05 GB + fp16 reference 3.79 GB; both-reference option argmax 58/58, both engines 232/232; measured through coreai-kit 58/58 parity, SemIf authored144 balanced 0.798 |
| [`system-one-scorer-4b`](system-one-scorer-4b/README.md) | pngwn/system-one-qwen3.5-4b-scorer (English scalar option scorer; Choice / noul / ordered Score, T=1.75; cc-by-nc-4.0) | [🤗 system-one-qwen3.5-4b-scorer-CoreAI](https://huggingface.co/mlboydaisuke/system-one-qwen3.5-4b-scorer-CoreAI) | int8lin 5.10 GB + fp16 reference 8.44 GB; option-question argmax 48/48 both, engines 8/8 load calls; measured through coreai-kit: tokens/slots 280/280 and question argmax 48/48 both; int8lin max \|Δp\| 0.0110, 1,016 ms/question (contended); SemIf authored144 mean family balanced accuracy 0.844; kit catalog entry pending the Hub revision |
| **CC BY-NC 4.0** · [`openjev-27b`](openjev-27b/README.md) | OpenJev (openjev/openjev; text-only 27B typed decisions, bare A–Z/a–z, T=0.85, calibrated Noul) | [🤗 OpenJev-CoreAI](https://huggingface.co/mlboydaisuke/OpenJev-CoreAI) | int8hu 27.8 GiB; Mac GPU argmax 61/61, engines 122/122; the AOT python readout gate peaked at 111.9 GB RSS; the kit's sequential engine holds it at about 28 GB resident; measured through coreai-kit: parity 61/61, SemIf authored144 balanced 0.907, median decision 8.3 s contended |
...
| [`agents-a1-4b`](agents-a1-4b/README.md) | Agents-A1-4B (InternScience; long-horizon agentic model on the Qwen3.5-4B text tower — the 4B recipe with the HF id swapped) | [🤗 Agents-A1-4B-CoreAI](https://huggingface.co/mlboydaisuke/Agents-A1-4B-CoreAI) | int8hu 5.77 GB (Mac-class, pipelined GPU), oracle gate **16/16** on two prompts, chat no-think stops on `<|im_end|>` 8/8, tool calls **3/3** in the template's XML `<function=…>` format, 8/8 sanity; the template injects a dated default system prompt when none is given (card) |
