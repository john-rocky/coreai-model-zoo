...
swift run -c release chat-cli --model qwen3.6-27b --prompt "What can you do, offline?"
```
...
if id == "qwen3-0.6b" {
    ...
    chat = try await ChatSession(catalog: "qwen3.6-27b")
}
...
- First run downloads the model — ~29,803 MB (Mac) — then it loads from the
